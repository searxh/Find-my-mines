  ...
  return (
    <div className="m-auto text-white">
      ...
      <div className="text-lg mx-auto w-[90%]">
        If the revealed grid contains a mine, the player will be rewarded with
        points. The game will end after all mines have been found. Different
        types of mines will reward different amount of points.
      </div>
      <div className="flex flex-col text-lg p-5">
        <div className="flex flex-row-reverse justify-evenly w-full">
          {Object.keys(defaultMinesConfig).map((mineKey, index) => (
            <div
              className={`${
                index === 0
                  ? "text-lime-400"
            ...
            >
              ...
              />
              <div>{defaultMinesConfig[mineKey].points}</div>
              <div>POINTS</div>
            </div>
          ))}
        </div>
