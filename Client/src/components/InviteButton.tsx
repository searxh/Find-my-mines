...
import { FlagsType, InviteMessageType, PriorityType, UserType } from "../types";
...
export default function InviteButton({
    ...
    const checkCanInvite = () => {
        return (
            !activeUsers.find(
                (activeUser: UserType) => activeUser.name === user.name
    ...
    React.useEffect(() => {
        ...
        if (Object.keys(receivedInvite).length !== 0) {
            ...
            if (check !== undefined) {
                ...
                console.log(
                    "i got a reply, removing pending invite for",
                    user.name
                );
                const newPendingInvite = { ...pendingInvite };
