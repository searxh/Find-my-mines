...
const removeExpiredInvitation = () => {
	const expiredKeys = Object.keys(invitation).filter((key) =>
...
const expireInvitation = (senderName) => {
	const expiredKeys = Object.keys(invitation).filter(
		(key) => invitation[key].senderName === senderName
	);
...
socketIO.of("/").adapter.on("leave-room", (roomID, id) => {
	...
	if (
		roomID.length > 20 &&
		(info === null || info === void 0 ? void 0 : info.state) === 2
	) {
...
socketIO.on("connection", (socket) => {
	...
	socket.on(
		"invite request",
		({ senderName, receiverName, inviteMessage, gameOptions }) => {
	...
	socket.on("chat request", ({ name, roomID }) => {
		...
		if (
			((_a = activeUsers[name]) === null || _a === void 0
				? void 0
				: _a.inGame) &&
			roomID !== undefined
		) {
